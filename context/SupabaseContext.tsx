...
export const SupabaseProvider = ({ children }: any) => {
   ...
   const createBoard = async (title: string, background: string) => {
      const { data, error } = await client.from(BOARDS_TABLE).insert({ title, creator: userId, background })
   ...
   const getBoardInfo = async (boardId: string) => {
      const { data } = await client.from(BOARDS_TABLE).select(`*, users (first_name)`).match({ id: boardId }).single()
      return data
   }
   ...
   const getBoardLists = async (boardId: string) => {
      const lists = await client.from(LISTS_TABLE).select("*").eq("board_id", boardId).order("position")

      return lists.data || []
   }

   const addBoardList = async (boardId: string, title: string, position = 0) => {
      return await client.from(LISTS_TABLE).insert({ board_id: boardId, position, title }).select("*").single()
   }
   ...
   const getListCards = async (listId: string) => {
      const lists = await client.from(CARDS_TABLE).select("*").eq("list_id", listId).eq("done", false).order("position")
   ...
   const getCardInfo = async (id: string) => {
      const { data } = await client.from(CARDS_TABLE).select(`*, users (*), boards(*)`).match({ id }).single()
      return data
   }
   ...
   const getBoardMember = async (boardId: string) => {
      const { data } = await client.from(USER_BOARDS_TABLE).select("users(*)").eq("board_id", boardId)
   ...
   const getRealtimeCardSubscription = (
      ...
      return client
         ...
         .on("postgres_changes", { event: "*", schema: "public", table: CARDS_TABLE }, handleRealtimeChanges)
         .subscribe()
   }
   ...
   const setUserPushToken = async (token: string) => {
      const { data, error } = await client.from(USERS_TABLE).upsert({ id: userId, push_token: token })
